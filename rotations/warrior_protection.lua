...
kps.rotations.register("WARRIOR","PROTECTION",
{
    ...
    {{"nested"}, 'activeEnemies.count >= 2', { -- call_action_list,name=aoe,if=active_enemies>=2
        ...
        {spells.thunderClap, 'target.myDebuffDuration(spells.deepWounds) < 3 and activeEnemies.count > 4'}, -- thunder_clap,cycle_targets=1,if=dot.deep_wounds.remains<3&active_enemies>4
        {spells.bladestorm, 'not player.hasBuff(spells.shieldCharge)'}, -- bladestorm,if=buff.shield_charge.down
        {spells.execute, 'player.buffStacks(spells.suddenDeath)'}, -- execute,if=buff.sudden_death.react
        {spells.thunderClap, 'activeEnemies.count > 6'}, -- thunder_clap,if=active_enemies>6

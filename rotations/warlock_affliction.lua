...
-- ERROR in 'service_pet,if=talent.grimoire_of_service.enabled&(target.time_to_die>120|target.time_to_die<=25|(buff.dark_soul.remains&target.health.pct<20))': Spell 'servicePet' unknown!
    ...
    {spells.soulburn, 'not player.hasTalent(7, 1) and activeEnemies.count > 2 and target.myDebuffDuration(spells.corruption) <= player.myDebuffDurationMax(spells.corruption) * 0.3'}, -- soulburn,cycle_targets=1,if=!talent.soulburn_haunt.enabled&active_enemies>2&dot.corruption.remains<=dot.corruption.duration*0.3
    {spells.seedOfCorruption, 'not player.hasTalent(7, 1) and activeEnemies.count > 2 and not target.myDebuffDuration(spells.seedOfCorruption) and player.buffDuration(spells.soulburn)'}, -- seed_of_corruption,cycle_targets=1,if=!talent.soulburn_haunt.enabled&active_enemies>2&!dot.seed_of_corruption.remains&buff.soulburn.remains
    ...
    {spells.soulburn, 'player.soulShards>0 and player.hasTalent(7, 1) and not player.hasBuff(spells.soulburn) and ( player.buffDuration(spells.hauntingSpirits) <= player.buffDurationMax(spells.hauntingSpirits) * 0.3 )'}, -- soulburn,if=shard_react&talent.soulburn_haunt.enabled&buff.soulburn.down&(buff.haunting_spirits.remains<=buff.haunting_spirits.duration*0.3)
    {spells.haunt, 'player.soulShards>0 and player.hasTalent(7, 1) and not spells.haunt.isRecastAt("target") and ( ( player.hasBuff(spells.soulburn) and ( ( player.buffDuration(spells.hauntingSpirits) <= player.buffDurationMax(spells.hauntingSpirits) * 0.3 and target.myDebuffDuration(spells.haunt) <= player.myDebuffDurationMax(spells.haunt) * 0.3 ) or not player.hasBuff(spells.hauntingSpirits) ) ) )'}, -- haunt,if=shard_react&talent.soulburn_haunt.enabled&!in_flight_to_target&((buff.soulburn.up&((buff.haunting_spirits.remains<=buff.haunting_spirits.duration*0.3&dot.haunt.remains<=dot.haunt.duration*0.3)|buff.haunting_spirits.down)))
    ...
    {spells.seedOfCorruption, 'not player.hasTalent(7, 1) and activeEnemies.count > 3 and not target.hasMyDebuff(spells.seedOfCorruption)'}, -- seed_of_corruption,cycle_targets=1,if=!talent.soulburn_haunt.enabled&active_enemies>3&!dot.seed_of_corruption.ticking
    ...
    {spells.seedOfCorruption, 'activeEnemies.count > 3 and not target.hasMyDebuff(spells.seedOfCorruption)'}, -- seed_of_corruption,cycle_targets=1,if=active_enemies>3&!dot.seed_of_corruption.ticking

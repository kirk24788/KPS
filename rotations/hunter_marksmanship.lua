...
kps.rotations.register("HUNTER","MARKSMANSHIP",
{
    ...
    {{"nested"}, 'player.hasBuff(spells.carefulAim)', { -- call_action_list,name=careful_aim,if=buff.careful_aim.up
        {spells.glaiveToss, 'activeEnemies.count > 2'}, -- glaive_toss,if=active_enemies>2
        {spells.powershot, 'activeEnemies.count > 1 and (player.focusMax-player.focus)'}, -- powershot,if=active_enemies>1&cast_regen<focus.deficit

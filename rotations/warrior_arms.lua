...
-- SKIP 'recklessness,if=(((target.time_to_die>190|target.health.pct<20)&(buff.bloodbath.up|!talent.bloodbath.enabled))|target.time_to_die<=12|talent.anger_management.enabled)&((desired_targets=1&!raid_event.adds.exists)|!talent.bladestorm.enabled)': Line Skipped
    ...
    {{"nested"}, 'activeEnemies.count == 1', { -- call_action_list,name=single,if=active_enemies=1
        ...
        {spells.dragonRoar, 'not player.hasBuff(spells.colossusSmash) and ( not activeEnemies.count or ( player.hasTalent(7, 1) and 0 ) )'}, -- dragon_roar,if=!debuff.colossus_smash.up&(!raid_event.adds.exists|raid_event.adds.in>55|(talent.anger_management.enabled&raid_event.adds.in>40))
    ...
    {{"nested"}, 'activeEnemies.count > 1', { -- call_action_list,name=aoe,if=active_enemies>1
        ...
        {spells.execute, 'not player.buffStacks(spells.suddenDeath) and activeEnemies.count <= 8 and ( ( player.rage > 72 and spells.colossusSmash.cooldown > player.gcd ) or player.rage > 80 or target.timeToDie < 5 or player.hasBuff(spells.colossusSmash) )'}, -- execute,cycle_targets=1,if=!buff.sudden_death.react&active_enemies<=8&((rage>72&cooldown.colossus_smash.remains>gcd)|rage>80|target.time_to_die<5|debuff.colossus_smash.up)
        {spells.mortalStrike, 'target.hp > 20 and activeEnemies.count <= 5'}, -- mortal_strike,if=target.health.pct>20&active_enemies<=5
        {spells.dragonRoar, 'not player.hasBuff(spells.colossusSmash)'}, -- dragon_roar,if=!debuff.colossus_smash.up
        {spells.thunderClap, '( target.hp > 20 or activeEnemies.count >= 9 ) and player.hasGlyph(spells.glyphOfResonatingPower)'}, -- thunder_clap,if=(target.health.pct>20|active_enemies>=9)&glyph.resonating_power.enabled
        {spells.rend, '(target.myDebuffDuration(spells.rend)/spells.rend.tickTime) < 2 and target.timeToDie > 8 and not player.hasBuff(spells.colossusSmash) and activeEnemies.count >= 9 and player.rage < 50 and not player.hasTalent(3, 1)'}, -- rend,cycle_targets=1,if=ticks_remain<2&target.time_to_die>8&!buff.colossus_smash_up.up&active_enemies>=9&rage<50&!talent.taste_for_blood.enabled
        {spells.whirlwind, 'target.hp > 20 or activeEnemies.count >= 9'}, -- whirlwind,if=target.health.pct>20|active_enemies>=9
        {spells.siegebreaker}, -- siegebreaker

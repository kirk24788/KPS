...
kps.rotations.register("PALADIN","PROTECTION",
{
    ...
    {spells.avengersShield, 'player.buffStacks(spells.grandCrusader) and activeEnemies.count > 1 and not player.hasGlyph(spells.glyphOfFocusedShield)'}, -- avengers_shield,if=buff.grand_crusader.react&active_enemies>1&!glyph.focused_shield.enabled
...
-- ERROR in 'judgment,cycle_targets=1,if=glyph.double_jeopardy.enabled&last_judgment_target!=target': Unknown expression 'last_judgment_target'!
    ...
    {spells.avengersShield, 'activeEnemies.count > 1 and not player.hasGlyph(spells.glyphOfFocusedShield)'}, -- avengers_shield,if=active_enemies>1&!glyph.focused_shield.enabled

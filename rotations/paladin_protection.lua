...
kps.rotations.register("PALADIN","PROTECTION",
{
    ...
    {spells.divineProtection, 'player.timeInCombat < 5 or not player.hasTalent(7, 2) or ( target.hasMyDebuff(spells.seraphim) and spells.seraphim.cooldown > 5 and spells.seraphim.cooldown < 9 )'}, -- divine_protection,if=time<5|!talent.seraphim.enabled|(buff.seraphim.down&cooldown.seraphim.remains>5&cooldown.seraphim.remains<9)
    {spells.guardianOfAncientKings, 'player.timeInCombat < 5 or ( target.hasMyDebuff(spells.holyAvenger) and target.hasMyDebuff(spells.shieldOfTheRighteous) and target.hasMyDebuff(spells.divineProtection) )'}, -- guardian_of_ancient_kings,if=time<5|(buff.holy_avenger.down&buff.shield_of_the_righteous.down&buff.divine_protection.down)
    {spells.ardentDefender, 'player.timeInCombat < 5 or ( target.hasMyDebuff(spells.holyAvenger) and target.hasMyDebuff(spells.shieldOfTheRighteous) and target.hasMyDebuff(spells.divineProtection) and target.hasMyDebuff(spells.guardianOfAncientKings) )'}, -- ardent_defender,if=time<5|(buff.holy_avenger.down&buff.shield_of_the_righteous.down&buff.divine_protection.down&buff.guardian_of_ancient_kings.down)
    ...
    {spells.sealOfRighteousness, 'player.hasTalent(7, 1) and not player.hasBuff(spells.sealOfRighteousness) and player.buffDuration(spells.uthersInsight) > spells.judgment.cooldown and target.hasMyDebuff(spells.liadrinsRighteousness)'}, -- seal_of_righteousness,if=talent.empowered_seals.enabled&!seal.righteousness&buff.uthers_insight.remains>cooldown.judgment.remains&buff.liadrins_righteousness.down

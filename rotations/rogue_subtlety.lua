...
kps.rotations.register("ROGUE","SUBTLETY",
{
    ...
    {spells.shadowDance, 'player.energy >= 50 and target.hasMyDebuff(spells.stealth) and target.hasMyDebuff(spells.vanish) and target.hasMyDebuff(spells.findWeakness) or ( player.bloodlust and ( target.hasMyDebuff(spells.hemorrhage) or target.hasMyDebuff(spells.garrote) or target.hasMyDebuff(spells.rupture) ) )'}, -- shadow_dance,if=energy>=50&buff.stealth.down&buff.vanish.down&debuff.find_weakness.down|(buff.bloodlust.up&(dot.hemorrhage.ticking|dot.garrote.ticking|dot.rupture.ticking))
    {spells.vanish, 'player.hasTalent(1, 3) and player.energy >= 45 and player.energy <= 75 and target.comboPoints < 4 - player.hasTalent(6, 3) and target.hasMyDebuff(spells.shadowDance) and target.hasMyDebuff(spells.masterOfSubtlety) and target.hasMyDebuff(spells.findWeakness)'}, -- vanish,if=talent.shadow_focus.enabled&energy>=45&energy<=75&combo_points<4-talent.anticipation.enabled&buff.shadow_dance.down&buff.master_of_subtlety.down&debuff.find_weakness.down
    {spells.vanish, 'player.hasTalent(1, 2) and player.energy >= 90 and target.comboPoints < 4 - player.hasTalent(6, 3) and target.hasMyDebuff(spells.shadowDance) and target.hasMyDebuff(spells.masterOfSubtlety) and target.hasMyDebuff(spells.findWeakness)'}, -- vanish,if=talent.subterfuge.enabled&energy>=90&combo_points<4-talent.anticipation.enabled&buff.shadow_dance.down&buff.master_of_subtlety.down&debuff.find_weakness.down
    {spells.markedForDeath, 'target.comboPoints == 0'}, -- marked_for_death,if=combo_points=0
    {{"nested"}, 'target.comboPoints == 5 and ( target.hasMyDebuff(spells.vanish) or not player.hasTalent(1, 3) )', { -- run_action_list,name=finisher,if=combo_points=5&(buff.vanish.down|!talent.shadow_focus.enabled)

...
kps.rotations.register("DEATHKNIGHT","UNHOLY",
{
    ...
    {{"nested"}, 'player.hasTalent(7, 3)', { -- run_action_list,name=bos,if=talent.breath_of_sindragosa.enabled
        ...
        {{"nested"}, 'True', { -- run_action_list,name=bos_st
            ...
            {{"nested"}, 'target.hasMyDebuff(spells.breathOfSindragosa)', { -- run_action_list,name=bos_active,if=dot.breath_of_sindragosa.ticking
                {spells.plagueStrike, 'not diseaseTicking()'}, -- plague_strike,if=!disease.ticking
    ...
    {{"nested"}, '( not player.hasTalent(7, 1) and activeEnemies() < 2 ) or activeEnemies() < 4', { -- run_action_list,name=single_target,if=(!talent.necrotic_plague.enabled&active_enemies<2)|active_enemies<4
        ...
        {spells.plagueStrike, 'diseaseTicking() and player.unholyRunes == 2'}, -- plague_strike,if=!disease.min_ticking&unholy=2
        {spells.scourgeStrike, 'player.unholyRunes == 2'}, -- scourge_strike,if=unholy=2
        ...
        {spells.plagueStrike, 'diseaseTicking() and ( player.bloodRunes == 2 or player.frostRunes == 2 )'}, -- plague_strike,if=!disease.min_ticking&(blood=2|frost=2)

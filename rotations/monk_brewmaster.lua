...
kps.rotations.register("MONK","BREWMASTER",
{
    ...
    {spells.chiBrew, '( player.chi < 1 and player.staggerPercent >= 0.065 ) or ( player.chi < 2 and target.hasMyDebuff(spells.shuffle) )'}, -- chi_brew,if=(chi<1&stagger.heavy)|(chi<2&buff.shuffle.down)
    ...
    {spells.diffuseMagic, 'kps.incomingDamage(1.5) and target.hasMyDebuff(spells.fortifyingBrew)'}, -- diffuse_magic,if=incoming_damage_1500ms&buff.fortifying_brew.down
    ...
    {{"nested"}, 'activeEnemies() < 3', { -- call_action_list,name=st,if=active_enemies<3
        ...
        {spells.zenSphere, 'not target.hasMyDebuff(spells.zenSphere) and player.energyTimeToMax > 2 and target.hasMyDebuff(spells.serenity)'}, -- zen_sphere,cycle_targets=1,if=!dot.zen_sphere.ticking&energy.time_to_max>2&buff.serenity.down
    ...
    {{"nested"}, 'activeEnemies() >= 3', { -- call_action_list,name=aoe,if=active_enemies>=3
        ...
        {spells.blackoutKick, 'player.buffDuration(spells.shuffle) <= 3 and spells.kegSmash.cooldown >= player.gcd'}, -- blackout_kick,if=buff.shuffle.remains<=3&cooldown.keg_smash.remains>=gcd
        {spells.blackoutKick, 'player.hasBuff(spells.serenity)'}, -- blackout_kick,if=buff.serenity.up
        {spells.rushingJadeWind, 'player.chiMax - player.chi >= 1 and target.hasMyDebuff(spells.serenity)'}, -- rushing_jade_wind,if=chi.max-chi>=1&buff.serenity.down
        {spells.chiBurst, 'player.energyTimeToMax > 2 and target.hasMyDebuff(spells.serenity)'}, -- chi_burst,if=energy.time_to_max>2&buff.serenity.down
        {spells.chiWave, 'player.energyTimeToMax > 2 and target.hasMyDebuff(spells.serenity)'}, -- chi_wave,if=energy.time_to_max>2&buff.serenity.down
        {spells.zenSphere, 'not target.hasMyDebuff(spells.zenSphere) and player.energyTimeToMax > 2 and target.hasMyDebuff(spells.serenity)'}, -- zen_sphere,cycle_targets=1,if=!dot.zen_sphere.ticking&energy.time_to_max>2&buff.serenity.down
        {spells.blackoutKick, 'player.chiMax - player.chi < 2'}, -- blackout_kick,if=chi.max-chi<2

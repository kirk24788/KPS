...
-- ERROR in 'blade_flurry,if=(spell_targets.blade_flurry>=2&!buff.blade_flurry.up)|(spell_targets.blade_flurry<2&buff.blade_flurry.up)': Unknown expression 'spell_targets.blade_flurry'!
    ...
    {{"nested"}, 'spells.killingSpree.cooldown > 10', { -- call_action_list,name=adrenaline_rush,if=cooldown.killing_spree.remains>10
        ...
        {spells.adrenalineRush, 'target.timeToDie <= 22.5'}, -- adrenaline_rush,if=target.time_to_die<=buff.adrenaline_rush.duration*1.5
    }},
    {{"nested"}, '( player.energy < 40 or ( player.bloodlust and player.timeInCombat < 10 )) and not player.hasBuff(spells.adrenalineRush) and ( not player.hasTalent(7, 2) or spells.shadowReflection.cooldown > 30 or player.buffDuration(spells.shadowReflection) > 3 )', { -- call_action_list,name=killing_spree,if=(energy<40|(buff.bloodlust.up&time<10)|buff.bloodlust.remains>20)&buff.adrenaline_rush.down&(!talent.shadow_reflection.enabled|cooldown.shadow_reflection.remains>30|buff.shadow_reflection.remains>3)
...
-- ERROR in 'killing_spree,if=target.time_to_die<44&trinket.stacking_proc.any.react&trinket.stacking_proc.any.remains>=buff.killing_spree.duration': Unknown expression 'trinket.stacking_proc.any.remains'!
        {spells.killingSpree, 'target.timeToDie <= 22.5'}, -- killing_spree,if=target.time_to_die<=buff.killing_spree.duration*1.5
    }},

...
kps.rotations.register("ROGUE","COMBAT",
{
    ...
    {spells.vanish, 'player.timeInCombat > 10 and ( target.comboPoints < 3 or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 3 ) or ( target.comboPoints < 4 or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 4 ) ) ) and ( ( player.hasTalent(1, 3) and target.hasMyDebuff(spells.adrenalineRush) and player.energy < 90 and player.energy >= 15 ) or ( player.hasTalent(1, 2) and player.energy >= 90 ) or ( not player.hasTalent(1, 3) and not player.hasTalent(1, 2) and player.energy >= 60 ) )'}, -- vanish,if=time>10&(combo_points<3|(talent.anticipation.enabled&anticipation_charges<3)|(combo_points<4|(talent.anticipation.enabled&anticipation_charges<4)))&((talent.shadow_focus.enabled&buff.adrenaline_rush.down&energy<90&energy>=15)|(talent.subterfuge.enabled&energy>=90)|(!talent.shadow_focus.enabled&!talent.subterfuge.enabled&energy>=60))
    {spells.sliceAndDice, 'player.buffDuration(spells.sliceAndDice) < 2 or ( ( target.timeToDie > 45 and target.comboPoints == 5 and player.buffDuration(spells.sliceAndDice) < 12 ) and target.hasMyDebuff(spells.deepInsight) )'}, -- slice_and_dice,if=buff.slice_and_dice.remains<2|((target.time_to_die>45&combo_points=5&buff.slice_and_dice.remains<12)&buff.deep_insight.down)
    ...
    {{"nested"}, '( player.energy < 40 or ( player.bloodlust and player.timeInCombat < 10 ) or player.buffDuration(spells.bloodlust) > 20 ) and target.hasMyDebuff(spells.adrenalineRush) and ( not player.hasTalent(7, 2) or spells.shadowReflection.cooldown > 30 or player.buffDuration(spells.shadowReflection) > 3 )', { -- call_action_list,name=killing_spree,if=(energy<40|(buff.bloodlust.up&time<10)|buff.bloodlust.remains>20)&buff.adrenaline_rush.down&(!talent.shadow_reflection.enabled|cooldown.shadow_reflection.remains>30|buff.shadow_reflection.remains>3)
...
-- ERROR in 'killing_spree,if=target.time_to_die<44&trinket.stacking_proc.any.react&trinket.stacking_proc.any.remains>=buff.killing_spree.duration': Unknown expression 'trinket.stacking_proc.any.remains'!
    ...
    {{"nested"}, 'target.comboPoints < 5 or not target.hasMyDebuff(spells.revealingStrike) or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 3 and target.hasMyDebuff(spells.deepInsight) )', { -- call_action_list,name=generator,if=combo_points<5|!dot.revealing_strike.ticking|(talent.anticipation.enabled&anticipation_charges<3&buff.deep_insight.down)

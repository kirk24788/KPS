...
kps.rotations.register("PALADIN","RETRIBUTION",
{
    ...
    {{"nested"}, 'activeEnemies() >= 3', { -- call_action_list,name=cleave,if=active_enemies>=3
        ...
        {spells.hammerOfTheRighteous, 'activeEnemies() >= 4 and ( player.holyPower <= 3 or ( player.holyPower == 4 and target.hp >= 35 and not player.hasBuff(spells.avengingWrath) ) )'}, -- hammer_of_the_righteous,if=active_enemies>=4&(holy_power<=3|(holy_power=4&target.health.pct>=35&buff.avenging_wrath.down))

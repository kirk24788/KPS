...
-- ERROR in 'counterspell,if=target.debuff.casting.react': Spell 'kps.spells.mage.casting' unknown (in expression: 'target.debuff.casting.react')!
    ...
    {spells.iceFloes, 'not player.hasBuff(spells.iceFloes) and ( player.isMoving or player.isMoving )'}, -- ice_floes,if=buff.ice_floes.down&(raid_event.movement.distance>0|raid_event.movement.in<action.fireball.cast_time)
    ...
    {{"nested"}, 'pyroChain() and ( activeEnemies.count > 1 or ( player.hasTalent(7, 2) and spells.prismaticCrystal.cooldown > 15 ) )', { -- call_action_list,name=t17_2pc_combust,if=set_bonus.tier17_2pc&pyro_chain&(active_enemies>1|(talent.prismatic_crystal.enabled&cooldown.prismatic_crystal.remains>15))
...
-- ERROR in 'pyroblast,if=prev_gcd.inferno_blast&execute_time=gcd.max&dot.ignite.tick_dmg*(6-ceil(dot.ignite.remains-travel_time))*100<hit_damage*(100+crit_pct_current)*mastery_value': Unknown expression 'dot.ignite.tick_dmg'!
        ...
        {spells.meteor, 'activeEnemies.count <= 2 and spells.pyroblast.isRecastAt("target")'}, -- meteor,if=active_enemies<=2&prev_gcd.pyroblast
...
-- ERROR in 'fireball,if=buff.pyroblast.up&buff.heating_up.down&dot.ignite.tick_dmg*100*(execute_time+travel_time)<hit_damage*(100+crit_pct_current)*mastery_value&(!current_target=prismatic_crystal|pet.prismatic_crystal.remains>6)': Unknown expression 'dot.ignite.tick_dmg'!
        ...
        {spells.infernoBlast, 'not player.hasBuff(spells.pyroblast) and not player.hasBuff(spells.heatingUp) and spells.pyroblast.isRecastAt("target")'}, -- inferno_blast,if=buff.pyroblast.down&buff.heating_up.down&prev_gcd.pyroblast
        {spells.fireball}, -- fireball
    }},
    {{"nested"}, 'pyroChain()', { -- call_action_list,name=combust_sequence,if=pyro_chain
        {spells.prismaticCrystal}, -- prismatic_crystal
        {spells.meteor, 'activeEnemies.count <= 2'}, -- meteor,if=active_enemies<=2
        {spells.pyroblast, 'player.hasBuff(spells.pyromaniac)'}, -- pyroblast,if=set_bonus.tier17_4pc&buff.pyromaniac.up
...
-- ERROR in 'inferno_blast,if=dot.ignite.tick_dmg*(6-dot.ignite.ticks_remain)<crit_damage*mastery_value': Unknown expression 'dot.ignite.tick_dmg'!
    ...
    {{"nested"}, 'player.hasTalent(7, 2) and pet.npcId == 76933', { -- call_action_list,name=crystal_sequence,if=talent.prismatic_crystal.enabled&pet.prismatic_crystal.active
        ...
        {{"nested"}, 'True', { -- call_action_list,name=single_target
            ...
            {spells.infernoBlast, '( spells.combustion.cooldown % 8 + spells.infernoBlast.charges >= 2 or not or not ( activeEnemies.count > 1 or player.hasTalent(7, 2) ) ) and not player.hasBuff(spells.pyroblast) and player.hasBuff(spells.heatingUp)'}, -- inferno_blast,if=(cooldown.combustion.remains%8+charges_fractional>=2|!set_bonus.tier17_2pc|!(active_enemies>1|talent.prismatic_crystal.enabled))&buff.pyroblast.down&buff.heating_up.up
            {{"nested"}, 'True', { -- call_action_list,name=active_talents
                {spells.meteor, 'activeEnemies.count >= 3 or ( player.hasGlyph(spells.glyphOfCombustion) and ( not player.hasTalent(6, 3) or player.buffStacks(spells.incantersFlow) + incanters_flow_direction >= 4 ) and spells.meteor.cooldownTotal - spells.combustion.cooldown < 10 )'}, -- meteor,if=active_enemies>=3|(glyph.combustion.enabled&(!talent.incanters_flow.enabled|buff.incanters_flow.stack+incanters_flow_dir>=4)&cooldown.meteor.duration-cooldown.combustion.remains<10)
                {{"nested"}, 'player.hasTalent(5, 1) and ( activeEnemies.count > 1|0 )', { -- call_action_list,name=living_bomb,if=talent.living_bomb.enabled&(active_enemies>1|raid_event.adds.in<10)
    ...
    {{"nested"}, 'activeEnemies.count > 10', { -- call_action_list,name=aoe,if=active_enemies>10

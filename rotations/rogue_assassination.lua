...
kps.rotations.register("ROGUE","ASSASSINATION",
{
    ...
    {spells.fanOfKnives, 'activeEnemies() > 2 and not target.hasMyDebuff(spells.deadlyPoison) and target.hasMyDebuff(spells.vendetta)'}, -- fan_of_knives,cycle_targets=1,if=active_enemies>2&!dot.deadly_poison_dot.ticking&debuff.vendetta.down
    {spells.dispatch, '( target.comboPoints < 5 or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 4 ) ) and activeEnemies() == 2 and not target.hasMyDebuff(spells.deadlyPoison) and target.hasMyDebuff(spells.vendetta)'}, -- dispatch,cycle_targets=1,if=(combo_points<5|(talent.anticipation.enabled&anticipation_charges<4))&active_enemies=2&!dot.deadly_poison_dot.ticking&debuff.vendetta.down
    ...
    {spells.mutilate, 'activeEnemies() == 2 and not target.hasMyDebuff(spells.deadlyPoison) and target.hasMyDebuff(spells.vendetta)'}, -- mutilate,cycle_targets=1,if=active_enemies=2&!dot.deadly_poison_dot.ticking&debuff.vendetta.down

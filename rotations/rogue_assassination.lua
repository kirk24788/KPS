...
kps.rotations.register("ROGUE","ASSASSINATION",
{
    ...
    {spells.rupture, 'activeEnemies.count > 1 and not target.hasMyDebuff(spells.rupture) and target.comboPoints == 5'}, -- rupture,cycle_targets=1,if=active_enemies>1&!ticking&combo_points=5
    ...
    {spells.crimsonTempest, 'target.comboPoints > 4 and activeEnemies.count >= 4 and target.myDebuffDuration(spells.crimsonTempest) < 8'}, -- crimson_tempest,if=combo_points>4&active_enemies>=4&remains<8
    {spells.fanOfKnives, '( target.comboPoints < 5 or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 4 ) ) and activeEnemies.count >= 4'}, -- fan_of_knives,if=(combo_points<5|(talent.anticipation.enabled&anticipation_charges<4))&active_enemies>=4
    {spells.rupture, '( target.myDebuffDuration(spells.rupture) < 2 or ( target.comboPoints == 5 and target.myDebuffDuration(spells.rupture) <= ( target.myDebuffDurationMax(spells.rupture) * 0.3 ) ) ) and activeEnemies.count == 1'}, -- rupture,if=(remains<2|(combo_points=5&remains<=(duration*0.3)))&active_enemies=1
    ...
    {spells.mutilate, 'target.hp > 35 and ( target.comboPoints < 4 or ( player.hasTalent(6, 3) and player.buffStacks(spells.anticipation) < 3 ) ) and activeEnemies.count < 5'}, -- mutilate,if=target.health.pct>35&(combo_points<4|(talent.anticipation.enabled&anticipation_charges<3))&active_enemies<5
    {spells.mutilate, 'activeEnemies.count == 2 and not target.hasMyDebuff(spells.deadlyPoison) and not player.hasBuff(spells.vendetta)'}, -- mutilate,cycle_targets=1,if=active_enemies=2&!dot.deadly_poison_dot.ticking&debuff.vendetta.down
    {spells.mutilate, 'activeEnemies.count < 5'}, -- mutilate,if=active_enemies<5
}

...
kps.rotations.register("WARRIOR","FURY",
{
    ...
    {spells.berserkerRage, 'not player.hasBuff(spells.enrage) or ( spells.bloodthirst.isRecastAt("target") and player.buffStacks(spells.ragingBlow) < 2 )'}, -- berserker_rage,if=buff.enrage.down|(prev_gcd.bloodthirst&buff.raging_blow.stack<2)
    ...
    {{"nested"}, '( < 60 and > 2 and activeEnemies.count == 1 ) or < 5', { -- call_action_list,name=single_target,if=(raid_event.adds.cooldown<60&raid_event.adds.count>2&active_enemies=1)|raid_event.movement.cooldown<5
        {spells.bloodbath}, -- bloodbath
        {spells.recklessness, 'target.hp < 20 and activeEnemies.count'}, -- recklessness,if=target.health.pct<20&raid_event.adds.exists
        {spells.wildStrike, '( player.rage > player.rageMax - 20 ) and target.hp > 20'}, -- wild_strike,if=(rage>rage.max-20)&target.health.pct>20
        {spells.bloodthirst, '( not player.hasTalent(3, 3) and ( player.rage < player.rageMax - 40 ) ) or not player.hasBuff(spells.enrage) or player.buffStacks(spells.ragingBlow) < 2'}, -- bloodthirst,if=(!talent.unquenchable_thirst.enabled&(rage<rage.max-40))|buff.enrage.down|buff.raging_blow.stack<2
        {spells.ravager, 'player.hasBuff(spells.bloodbath) or ( not player.hasTalent(6, 2) and ( not activeEnemies.count or 0 or target.timeToDie < 40 ) )'}, -- ravager,if=buff.bloodbath.up|(!talent.bloodbath.enabled&(!raid_event.adds.exists|raid_event.adds.in>60|target.time_to_die<40))
        ...
        {spells.bladestorm, 'not activeEnemies.count'}, -- bladestorm,if=!raid_event.adds.exists
    ...
    {spells.recklessness, '( ( ( target.timeToDie > 190 or target.hp < 20 ) and ( player.hasBuff(spells.bloodbath) or not player.hasTalent(6, 2) ) ) or target.timeToDie <= 12 or player.hasTalent(7, 1) ) and ( ( player.hasTalent(6, 3) and ( not activeEnemies.count or activeEnemies.count == 1 ) ) or not player.hasTalent(6, 3) )'}, -- recklessness,if=(((target.time_to_die>190|target.health.pct<20)&(buff.bloodbath.up|!talent.bloodbath.enabled))|target.time_to_die<=12|talent.anger_management.enabled)&((talent.bladestorm.enabled&(!raid_event.adds.exists|enemies=1))|!talent.bladestorm.enabled)
    ...
    {{"nested"}, 'activeEnemies.count == 1', { -- call_action_list,name=single_target,if=active_enemies=1
        {spells.bloodbath}, -- bloodbath
        {spells.recklessness, 'target.hp < 20 and activeEnemies.count'}, -- recklessness,if=target.health.pct<20&raid_event.adds.exists
        {spells.wildStrike, '( player.rage > player.rageMax - 20 ) and target.hp > 20'}, -- wild_strike,if=(rage>rage.max-20)&target.health.pct>20
        {spells.bloodthirst, '( not player.hasTalent(3, 3) and ( player.rage < player.rageMax - 40 ) ) or not player.hasBuff(spells.enrage) or player.buffStacks(spells.ragingBlow) < 2'}, -- bloodthirst,if=(!talent.unquenchable_thirst.enabled&(rage<rage.max-40))|buff.enrage.down|buff.raging_blow.stack<2
    ...
    {{"nested"}, 'activeEnemies.count == 2', { -- call_action_list,name=two_targets,if=active_enemies=2
        {spells.bloodbath}, -- bloodbath
...
-- SKIP 'bladestorm,if=buff.enrage.remains>6&((talent.anger_management.enabled&raid_event.adds.in>45)|(!talent.anger_management.enabled&raid_event.adds.in>60)|!raid_event.adds.exists|active_enemies>desired_targets)': Line Skipped
        ...
        {spells.bloodthirst, 'not player.hasBuff(spells.enrage) or player.rage < 40 or not player.hasBuff(spells.ragingBlow)'}, -- bloodthirst,if=buff.enrage.down|rage<40|buff.raging_blow.down
        {spells.siegebreaker}, -- siegebreaker
    ...
    {{"nested"}, 'activeEnemies.count == 3', { -- call_action_list,name=three_targets,if=active_enemies=3
        {spells.bloodbath}, -- bloodbath
        {spells.ravager, 'player.hasBuff(spells.bloodbath) or not player.hasTalent(6, 2)'}, -- ravager,if=buff.bloodbath.up|!talent.bloodbath.enabled
        {{"nested"}, 'True', { -- call_action_list,name=bladestorm
-- SKIP 'recklessness,sync=bladestorm,if=buff.enrage.remains>6&((talent.anger_management.enabled&raid_event.adds.in>45)|(!talent.anger_management.enabled&raid_event.adds.in>60)|!raid_event.adds.exists|active_enemies>desired_targets)': Line Skipped
-- SKIP 'bladestorm,if=buff.enrage.remains>6&((talent.anger_management.enabled&raid_event.adds.in>45)|(!talent.anger_management.enabled&raid_event.adds.in>60)|!raid_event.adds.exists|active_enemies>desired_targets)': Line Skipped
        ...
        {spells.bloodthirst, 'not player.hasBuff(spells.enrage) or player.rage < 50 or not player.hasBuff(spells.ragingBlow)'}, -- bloodthirst,if=buff.enrage.down|rage<50|buff.raging_blow.down
